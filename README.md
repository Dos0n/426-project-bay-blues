...
<!-- AI: Sprint 4 reviewer guidance added grader-facing startup and async verification commands. -->
## Sprint 4 Async Path

Start the complete simulation from the repository root:
...
Trigger and verify the RabbitMQ notification path, including queue retention
and redelivery after a worker failure:

```bash
./scripts/verify-sprint-4-async.sh
```

The verifier prints a `PASS` result for normal delivery, non-enqueueing
requests, queued work during a worker outage, redelivery after `SIGKILL`, and
the final drained queue.
<!-- AI: End Sprint 4 reviewer-guided startup and verification section. -->
