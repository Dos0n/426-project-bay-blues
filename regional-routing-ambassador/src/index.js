...
const proxyRequest = async (request, response) => {
  ...
  for (let attempt = 1; attempt <= maximumAttempts; attempt += 1) {
    ...
    try {
      ...
      if (upstreamResponse.status >= 500 && attempt < maximumAttempts) {
...
app.get("/health", async (_request, response) => {
  try {
    ...
    response.status(200).json({
      status: "ok",
      service: "regional-routing-ambassador",
      upstream: "regional-routing-service",
    });
  } catch {
    response.status(503).json({
      status: "degraded",
  ...
  if (request.method !== "GET" && request.method !== "HEAD") {
    response.status(405).json({
      error: {
        ...
        message: "Ambassador only proxies GET and HEAD requests",
      },
    });
    return;
  }

  proxyRequest(request, response).catch(next);
});
