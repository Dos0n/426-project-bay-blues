...
The two primary services shown here are separate client-facing paths in
Sprint 2; `incident-service` does not call `regional-routing-service`. Both
are only reachable through their own observable ambassador container:
`incident-ambassador` is the only path to `incident-service`, and
